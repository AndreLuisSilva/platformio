...
void init_SD_Card() {
  ...
  if (!sd_Card.begin(SS)) {
   ...
   File file = sd_Card.open("/data.txt");
  if(!file) {
    Serial.println("Arquivo ainda não existe");
    Serial.println("Criando arquivo...");   

   file = sd_Card.open("/data.txt", FILE_APPEND);
  ...
  Serial.println((float)sd_Card.cardSize()/1000);
 
  Serial.print("Total em bytes: ");
  Serial.println(sd_Card.totalBytes());
 
  Serial.print("Bytes utilizados: ");
  Serial.println(sd_Card.usedBytes()); 
