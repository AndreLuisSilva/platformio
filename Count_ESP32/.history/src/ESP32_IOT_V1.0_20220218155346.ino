...
#define LED_ON_OFF 27
#define LED_ATTENTION 26
...
//Laboratorio FORMATEC
//const char *ssid = "LABORATORIO";
//const char *password = "labo2022";
...
void TASK_Check_Internet(void * p) {
   ...
   //Delay de 1 segundos da tarefa. É feita em ticks. Para executar em millis dividimos pela constante portTICK_PERIOD_MS
...
void calibra_SCT013()
{
  //variavel local para receber estado do jumper
  int ler_Estado_Jumper = digitalRead(JUMPER_CALIBRAGEM);
  //delay para debounce
  delay(250);

  //se o estado do botão for nível lógico alto
  if (ler_Estado_Jumper == LOW)
  {
    Serial.println("Entrou no modo de calibragem da maquina!");
   
    //cria a task pisca led para indicar que esta no modo de calibragem
    xTaskCreatePinnedToCore(
      TASK_pisca_Led, //Função que será executada
      "TASK_pisca_Led", //Nome da tarefa
      10000,  //Tamanho da pilha
      NULL, //Parâmetro da tarefa (no caso não usamos)
      1,  //Prioridade da tarefa
      &HANDLE_pisca_Led,  //Caso queria manter uma referência para a tarefa que vai ser criada (no caso não precisamos)
      1); //Número do core que será executada a tarefa (usamos o core 0 para o loop ficar 

    //loop para fazer a calibração por 1 minuto(60 segundos)
    while (count_Seconds_Calibragem <= ARRAY_SIZE)
    {
      //faz a leitura da corrente e armazena e uma variavel
      double Irms = emon1.calcIrms(1480);
      //imprime na seria qual leitura esta sendo feita
      Serial.print(count_Seconds_Calibragem);
      Serial.print("ª leitura da calibragem = ");
      Serial.print(Irms);
      Serial.println(" A");
      //vetor na posição "n" do vetor salva o valor da leitura do sct013
      vetor_Calibragem[count_Seconds_Calibragem] = Irms;
      //contador para fazer uma leitura por segundo
      count_Seconds_Calibragem++;
      //delay de 1 segundo entre uma leitura e outra
      delay(1000);
    }

    //reiniciar o contador da calibragem
    count_Seconds_Calibragem = 1;
    //variavel para armazenar a soma das leituras
    double sum_Calibragem;
    //variavel para armazenar a media da calibragem
    uint32_t avg_Calibragem;

    //Faz a soma do vetor
    for (int i = 0; i < ARRAY_SIZE; i++)
    {
      sum_Calibragem = vetor_Calibragem[i] + sum_Calibragem;
    }

    //Realiza a media das 60 leituras para calibrar
    avg_Calibragem = sum_Calibragem / ARRAY_SIZE;

    Serial.println("");
    Serial.print(avg_Calibragem);
    Serial.println(" A");

    // Print the counter to Serial Monitor
    Serial.printf("Current counter value: %u\n", counter);
    counter = avg_Calibragem;
    preferences.begin("my-app", false);
    // Store the counter to the Preferences
    preferences.putUInt("counter", counter);
    // Close the Preferences
    preferences.end();
    //deleta a task pisca led pois só é utilizada enquanto está sendo calibrada
    Serial.print(counter);
    Serial.println(" A");
    //deleta a task pisca led
    vTaskDelete(HANDLE_pisca_Led);
    //deixa o led ligado mostrando que já terminou a calibragem e precisa ser alterado o jumper para o outro lado e reiniciado a placa
    while (true)
    {
      digitalWrite(LED_CALIBRAGEM, HIGH);
    }
