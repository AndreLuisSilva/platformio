...
//**************************************************************************
// Test function for negative temps
float RTC_DS3231::getTemp1()
{
  uint8_t lsb; // Made var "msb" a signed 8-bit type which allows float to retain sign
  char msb; // NOTE: char, but also +127/-128 signed 8-bit value
  Wire.beginTransmission(DS3231_ADDRESS);
  Wire._I2C_WRITE(DS3231_TEMPERATUREREG);
  Wire.endTransmission();
  ...
  return (float) msb + (lsb >> 6) * 0.25f; //NOTE: This is unchanged
}
